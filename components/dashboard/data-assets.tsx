    ...
    <Card className="h-full rounded-2xl border-slate-200 bg-slate-50/95 shadow-sm">
      ...
      <CardContent className="grid grid-cols-2 gap-3">
        {assetSummary.map((asset) => (
          <div key={asset.label} className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">{asset.label}</p>
            <p className="mt-1 text-2xl font-semibold text-slate-900">{asset.value}</p>
          </div>
        ))}
      </CardContent>
    </Card>
