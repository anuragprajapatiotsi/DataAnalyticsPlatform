        ...
        {isDatabase ? (
            <div className="flex flex-col gap-4">
              {testResult && (
                <div className={cn(
                  ...
                  <div className={cn(
                    ...
                    {testResult.success && <CheckCircle2 size={16} className="shrink-0" />}
              ...
              <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-4">
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] text-slate-500 mt-2 font-medium">
                    This connection string URL will be generated automatically on submission based on your inputs.
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 rounded-xl border border-slate-100">
                <Form.Item
                  name="host"
                  label={<span className="font-bold text-slate-700 text-xs uppercase tracking-wider">Host</span>}
                  rules={[{ required: true, message: "Host is required" }]}
                  className="col-span-1"
                >
                  <Input placeholder="e.g. localhost, db.example.com" className="h-10 border-slate-200 shadow-sm rounded-lg" />
                </Form.Item>

                <Form.Item
                  name="port"
                  label={<span className="font-bold text-slate-700 text-xs uppercase tracking-wider">Port</span>}
                  rules={[{ required: true, message: "Port is required" }]}
                  className="col-span-1"
                >
                  <InputNumber className="w-full h-10 border-slate-200 shadow-sm rounded-lg" controls={false} />
                </Form.Item>

                <Form.Item
                  name="database"
                  label={<span className="font-bold text-slate-700 text-xs uppercase tracking-wider">Database Name</span>}
                  rules={[{ required: true, message: "Database name is required" }]}
                  className="col-span-1 md:col-span-2"
                >
                  <Input placeholder="e.g. production_db" className="h-10 border-slate-200 shadow-sm rounded-lg" />
                </Form.Item>

                <Form.Item
                  name="user"
                  label={<span className="font-bold text-slate-700 text-xs uppercase tracking-wider">Username <span className="text-slate-400 font-normal lowercase">(optional)</span></span>}
                  className="col-span-1"
                >
                  <Input placeholder="Database user" className="h-10 border-slate-200 shadow-sm rounded-lg" />
                </Form.Item>

                <Form.Item
                  name="password"
                  label={<span className="font-bold text-slate-700 text-xs uppercase tracking-wider">Password <span className="text-slate-400 font-normal lowercase">(optional)</span></span>}
                  className="col-span-1"
                >
                  <Input.Password placeholder="Database password" className="h-10 border-slate-200 shadow-sm rounded-lg" />
                </Form.Item>
              </div>
            </div>
        ) : (
